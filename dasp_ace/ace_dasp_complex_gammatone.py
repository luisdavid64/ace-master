...
class OptimizedDASPACE(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # This processes the envelope, not the raw band signals
        ...
        sce_outputs = []
        for c in range(C):
            # Apply processed envelope to real parts with frequency signs
            modulated = ck_real[:, c:c+1, :, :] * freq_signs * sce_env.unsqueeze(1) / (env0[:, c:c+1, :, :] + regDelta)
            # Sum across bands
            reconstructed = modulated.sum(dim=2)  # [B, 1, T]
            sce_outputs.append(reconstructed)
        
        sce_output = torch.cat(sce_outputs, dim=1)  # [B, C, T]
        ...
        return {
            ...
            "sce_env": sce_env         # processed envelope from SCE
        }
